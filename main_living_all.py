...
def choice_number_rtpoints(threshold_vote,dict_vote,the_index,lptn, lpsn,rt_all):
    ...
    return rt,rt_all
def find_rgb_and_ply(names,ply_nlist, rgb_nlist):
    ...
    pcd_T = o3d.io.read_point_cloud(path_ply2)
    return pcd,pcd_T
...
def WriteRt(rt_all,pathRtxt,w_all):
    ...
    w_all.append(w)
    # WriteRttxt = open('./output/LivingAllRt.txt', 'w')                            # 转换rt成7组数,前三个为平移,后四个为四元
...
if __name__ == "__main__":
    ...
    names= getnames_living(data_path,jump=1)                                         #改，得到names、plynames，jump=1时不跳帧，jump=5时，50-55,55-60....
    ...
    w_all=[]
    for z in range(names.index('850')+1,len(names)-1):                                               #这里从第50帧开始拼names.index('50')+1,
        pcd,pcd_T=find_rgb_and_ply(names, ply_nlist, rgb_nlist)
        ...
        dict_vote, the_index, lpsn, lptn = B(path)                                    #choose B(path), or C(path)
        rt,rt_all=choice_number_rtpoints(500,dict_vote,the_index,lptn, lpsn,rt_all)                                         #改,筛点的阈值
        p_rt_all=TransformPointCloud(pcd_T,rt_all,p_rt_all)
        print('-' * 150)
    WriteRt(rt_all, './output/LivingAllRt.txt',w_all)                                   #改,是否写入rt转七元数
    WriteCameraXYZ(p_rt_all,'./output/cameraxyz.txt' )                            #改,是都记录相机坐标原点变化

    final = o3d.io.read_point_cloud(path_o + "living_room_png/ply/" + "477" + ".ply") #  改，起始帧
    for h in p_rt_all:                                                                # 把起始帧分别拼接之前算出的像第一帧转化的点云
        final += h
    o3d.io.write_point_cloud("v-livingroom_onebvone.ply", final, write_ascii=True)    #改，储存点云并命名
