...
logfile = relativePath()+'/registration_evaluate/heads_eval/head_output/voteC3.log'
# open的打开模式这里可以进行参考
...
def them(superglue_eval_txts):
    ...
    for i in range(fragmentsNum):
        for j in range(i + 1, fragmentsNum):
            ...
            timeStart = time.time()
            n, lps, lpt = A(txt_path)  # livingRoom需要除以10
            ...
            # 使用rt矩阵进行拼接并且可视化
            # ----------------------------------------------------------------------------------
            sourcePly = "/home/light/gree/align/eval/heads/heads_eval/heads/heads_evaluation/ply_eval/" + str(
                id_lst[1]) + ".ply"
            targetPly = "/home/light/gree/align/eval/heads/heads_eval/heads/heads_evaluation/ply_eval/" + str(
                id_lst[0]) + ".ply"
...
if __name__=="__main__":
    ...
    # ----------------------------------------------------------------------------------
    superglue_eval_txts= relativePath()+"/registration_evaluate/heads_eval/head_output/text3d/"
